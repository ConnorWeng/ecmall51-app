...
class BaseModel extends Object {
    ...
    function find($params = array()) {
        ...
        // echo '<br>table: '.$this->table. '--$this-alias: '.$this->alias;
        /* 左联结(LEFT JOIN) */
        $join_result = $this->_joinModel($tables, $join);
        // echo '<br>after table:'.$tables;
    ...
    /**
     * g.if_show = 1 AND g.closed = 0 AND s.state = 1 AND g.cate_id_1 = '16' AND s.mk_id IN ('2','23','24','25','26','27','28','29','30','31') AND s.mk_id = '23' AND g.realpic = 1 AND g.add_time <= 1426215598 AND g.add_time >= 1425610798 AND g.price >= '1' AND g.price <= '111' AND s.store_id IS NOT NULL
     * @param type $conditions
     * @param type $if_add_alias
     * @param type $g_con
     * @param type $s_con
     * @param type $gs_con
     * @param type $gst_con
     * @return string
     */
    function _getMyConditions($conditions, &$g_con, &$s_con, &$gs_con, &$gst_con) {
        if (is_string($conditions)) {
            $cons = preg_split("/AND/", $conditions);
            if (!$cons)
                return "";
            foreach ($cons as $con) {
                if (strpos($con, "gst.") !== false) {
                    $gst_con.= " AND " . $con;
                } else if (strpos($con, "gs.") !== false) {
                    $gs_con.= " AND " . $con;
                } else if (strpos($con, "g.") !== false) {
                    $g_con.= " AND " . $con;
                } else if (strpos($con, "s.") !== false) {
                    $s_con.= " AND " . $con;
                }
            }
        }
        $g_con = " where 1=1 " . $g_con;
        $s_con = " where 1=1 " . $s_con;
        $gs_con = " where 1=1 " . $gs_con;
        $gst_con = " where 1=1 " . $gst_con;
    }

    var $sphinxArray = array('attr_value','store_id','add_time', 'mk_id', 'recommended', 'moods', 'price', 'sort_order', 'cate_id_1',
        'realpic', 'serv_sendgoods', 'serv_refund', 'serv_exchgoods', 'sales', 'views', 'credit_value', 'cate_id_2', 'cate_id_3', 'cate_id_4',
        'spec_1', 'spec_2');
    var $sphinxOrderArray = array('add_time', 'mk_id', 'recommended', 'moods', 'price','sort_order', 'cate_id_1',
        'realpic', 'serv_sendgoods', 'serv_refund', 'serv_exchgoods', 'sales', 'views', 'credit_value', 'cate_id_2', 'cate_id_3', 'cate_id_4',
        'spec_1', 'spec_2');
    private function isValidCond($con) {
        foreach ($this->sphinxArray as $one) {
//             echo '<br> now ,we check $one '.$one.'  and the $con is '.$con;
            if (strpos($con, $one) !== false) {
//                echo '<br>we have the one'.$one;
                return $one;
            }
        }
        return false;
    }
   private function isValidSort($con) {
        foreach ($this->sphinxOrderArray as $one) {
            if (strpos($con, $one) !== false) {
                return $one;
            }
        }
    ...
    private function combinCon(&$reCondition, $r, $con) {
        if ($r) {
            if (strpos($con, ">=") !== false || strpos($con, "<=") !== false) {
//                echo '<br> here the range: '.$con;
                if (strpos($con, ">=") !== false) {
                    $pos = strpos($con, ">=");
                    $index = 'min';
                    $value = substr($con, $pos + 2);
                } else {
                    $pos = strpos($con, "<=");
                    $index = 'max';
                    $value = substr($con, $pos + 2);
                }
                if (!isset($reCondition[$r])) {
                    $rr = array();
                } else {
                    $rr = $reCondition[$r];
                }
//                echo 'value is :'.$value;
                $rr['type'] = 'range';
                $rr[$index] = intval(str_replace("'", '', $value));
                $rr['exclude'] = false;
                $reCondition[$r] = $rr;
            } else if (strpos($con, ">") !== false || strpos($con, "<") !== false) {
                if (strpos($con, ">") !== false) {
                    $pos = strpos($con, ">");
                    $index = 'min';
                } else {
                ...
                $rr['type'] = 'range';
                $rr[$index] = intval(str_replace("'", '', substr($con, $pos + 1)));
                $rr['exclude'] = true;
                $reCondition[$r] = $rr;
            } else if (strpos($con, "=") !== false) {
                if ($r == 'spec_1' || $r == 'spec_2') {
                    $rr = array();
                    $rr['type'] = 'keywords';
                    $rr['value'] = str_replace("'", '', substr($con, strpos($con, "=") + 1));
                    $reCondition[$r] = $rr;
                } else {
                    $rr = array();
                    $rr['type'] = 'equal';
                    $rr['value'] = intval(str_replace("'", '', substr($con, strpos($con, "=") + 1)));
                    $reCondition[$r] = $rr;
                }
            } else if (strpos($con, "IN") !== false || strpos($con, "in") !== false) {
                $pos = strpos($con, "(");
                $pos2 = strpos($con, ")");
                $values = str_replace("'", '', substr($con, $pos + 1, $pos2 - $pos - 1));
                $vars = split(',', $values);
                $rr = array();
                $rr['type'] = 'in';
                $rr['value'] = $vars;
    ...
    private function handleCon($con, &$reCondition) {
        $r = $this->isValidCond($con);
//        echo '$r: '.$r;
        $this->combinCon($reCondition, $r, $con);
    }

    /**
     * 思路　１：　生成 array(a=> array(type= equal; value=5)
     *                      b=> array(type=range ; min=1; max =5);
     * 然后使用setfilter&set filterRange
     * 思路２：　使用setselect  不成立，这个是获取字段
     * @param type $conditions
     * @param type $g_con
     * @param type $s_con
     * @param type $gs_con
     ...
     * @return string
     */
    function _getOneCondition($conditions) {
        $reCondition = array();
        if (is_string($conditions)) {
            $cons = preg_split("/AND/", $conditions);
//            print_r($cons);
            ...
            foreach ($cons as $con) {
                if (strpos($con, "gst.") !== false) {
                    $gst_con = substr($con, strpos($con, "gst.") + 4);
                    $this->handleCon($gst_con, $reCondition);
                } else if (strpos($con, "gs.") !== false) {
                    $gs_con = substr($con, strpos($con, "gs.") + 3);
                    $this->handleCon($gs_con, $reCondition);
                } else if (strpos($con, "g.") !== false) {
                    $g_con = substr($con, strpos($con, "g.") + 2);
                    $this->handleCon($g_con, $reCondition);
                } else if (strpos($con, "s.") !== false) {
                    $s_con = substr($con, strpos($con, "s.") + 2);
//                    echo '$s_con:'.$s_con;
                    $this->handleCon($s_con, $reCondition);
                } else {
                    $this->handleCon($con, $reCondition);
                }
            }
        }
        return $reCondition;
    ...
    function _getMyFields($orgfields, &$g_f, &$s_f, &$gs_f, &$gst_f) {
        if (is_string($orgfields)) {
            $fields = preg_split("/,/", $orgfields);
            if (!$fields)
                return "";
//            print_r($fields);
            foreach ($fields as $field) {
                if (strpos($field, "gst.") !== false) {
                    $gst_f.= "," . $field;
                } else if (strpos($field, "gs.") !== false) {
                    $gs_f.= "," . $field;
                } else if (strpos($field, "g.") !== false) {
                    $g_f.= "," . $field;
                } else if (strpos($field, "s.") !== false) {
                    $s_f.= "," . $field;
                }
            }
        }
        ...
        $s_f = substr($s_f, 1);
        $gs_f = substr($gs_f, 1);
        $gst_f = substr($gst_f, 1);
    }

    function _getMySorts($orgSorts, &$g_s, &$s_s, &$gs_s, &$gst_s, $g_f, $s_f, $gs_f, $gst_f) {

        if (is_string($orgSorts)) {
            $pos = strpos($orgSorts, 'ORDER BY');
            if ($pos !== false) {
                $orgSorts = substr($orgSorts, $pos + 8);
            }
...
//            print_r($fields);
            foreach ($fields as $field) {
                if (strpos($field, "gst.") !== false) {
                    ...
                    if (strpos($g_f, $this->clearit($field)) !== false) {
                        $g_s.= "," . $field;
                    } else if (strpos($s_f, $field) !== false) {
                        $s_s.= "," . $field;
                    } else if (strpos($gs_f, $field) !== false) {
                        $gs_s.= "," . $field;
                    } else if (strpos($gst_f, $field) !== false) {
                        $gst_s.= "," . $field;
                    }
                }
            }
        ...
        $g_s && $g_s = " ORDER BY " . substr($g_s, 1);
        $s_s && $s_s = " ORDER BY " . substr($s_s, 1);
        $gs_s && $gs_s = " ORDER BY " . substr($gs_s, 1);
        $gst_s && $gst_s = " ORDER BY " . substr($gst_s, 1);
    }

    function _getOneSort($orgSorts, $g_f, $s_f, $gs_f, $gst_f) {
        ...
        if (is_string($orgSorts)) {
            ...
            foreach ($fields as $field) {
                if (!$this->isValidSort($field))
                    continue;
                if (strpos($field, "gst.") !== false) {
                    $reOrder.= "," . substr($field, strpos($field, "gst.") + 4);
                } else if (strpos($field, "gs.") !== false) {
                    $reOrder.= "," . substr($field, strpos($field, "gs.") + 3);
                } else if (strpos($field, "g.") !== false) {
                    $reOrder.= "," . substr($field, strpos($field, "g.") + 2);
                } else if (strpos($field, "s.") !== false) {
                    $reOrder.= "," . substr($field, strpos($field, "s.") + 2);
                } else {
                    if (strpos($g_f, $this->clearit($field)) !== false) {
//                        echo '<br>here'.$field.'--'.$g_f;
                        $reOrder.= "," . $field;
//                         echo '<br> then '.$g_s;
                    } else if (strpos($s_f, $this->clearit($field)) !== false) {
                        $reOrder.= "," . $field;
                    } else if (strpos($gs_f, $this->clearit($field)) !== false) {
                        $reOrder.= "," . $field;
                    } else if (strpos($gst_f, $this->clearit($field)) !== false) {
                        $reOrder.= "," . $field;
                    }
                }
            }
        }
//            echo '<br> my g_s '.$g_s.'<br>';
//            $reOrder = "";
//            $g_s && $reOrder .= substr($g_s, 1);
//            $s_s && $reOrder .= $s_s;
//            $gs_s && $reOrder.= $gs_s;
//            $gst_s && $reOrder .= $gst_s;
        if (strpos($reOrder, ',') === 0)
