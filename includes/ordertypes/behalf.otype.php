...
class BehalfOrder extends BaseOrder {
    ...
    function submit_order($data) {
        ...
        if ($discount_result !== false) {
            if ($consignee_info['shipping_fee'] >= $discount_result)
                $consignee_info['shipping_fee'] -= $discount_result;
            else
    ...
    private function _calc_vip_discount($buyer_id, $bh_id) {
        ...
        return empty($discount_arr['vip' . $membervip_info['level']]) ? false : $discount_arr['vip' . $membervip_info['level']];
    }

    /**
     * 计算商品服务费（代发费、质检费、卡标、吊牌）
     *
     * @param unknown $post
     */
