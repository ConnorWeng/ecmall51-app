...
/*退款状态*/
define('REFUND_APPLYING',0);//申请退款，卖家处理前可修改
define('REFUND_FINISHED',1);//退款完成
define('REFUND_MODIFIED',2);//打回申请，重新修改
define('REFUND_PENDING',3);//同意退货退款，待买家发货
define('REFUND_SHIPPED',4);//同意退货退款，待卖家确认收货
...
/*备货状态*/
define('PREPARE_GOODS_PENDING', 11);//待付款
define('PREPARE_GOODS_ACCEPTED', 20);//已付款
define('PREPARE_GOODS_FINISH', 30);//货已备齐
define('PREPARE_GOODS_CANCEL', 0);//取消备货

/* 第三方订单状态 */
define('VENDOR_ORDER_UNHANDLED', 50);   // 第三方订单还未处理
define('VENDOR_ORDER_PENDING', 51);     // 第三方订单已下单未付款
define('VENDOR_ORDER_ACCEPTED', 52);    // 第三方订单已付款，等待发货
define('VENDOR_ORDER_SHIPPED', 53);    // 第三方订单已经发货
define('VENDOR_ORDER_SYNCED', 54);      // 第三方订单已同步回第三方平台
...
define('SUBMIT_ORDER_PARAMS_ERROR', 510004);
...
define('PARAMS_NOT_PROVIDED', 510012);
define('GOODS_NO_LOGISTICS', 510013);
define('NO_SUCH_GOODS', 510014);
define('PRICE_LESS_5', 510015);
define('CAN_NOT_BUY_YOURSELF', 510016);
define('GOODS_ALREADY_IN_CART', 510017);
define('NO_ENOUGH_GOODS', 510018);
define('CART_IS_FULL', 510019);
define('PHONE_ILLEAGE', 510020);
define('ZIPCODE_ILLEAGE', 510021);
define('REGION_ILLEAGE', 510022);
define('CONSIGNEE_REQUIRED', 510023);
define('ADDRESS_REQUIRED', 510024);
define('ADD_ADDRESS_ERROR', 510025);
define('DROP_FROM_CART_ERROR', 510026);
define('START_TRANSACTION_FAILED', 510027);
...
define('CHECK_TAOBAO_FAILED', 510034);
define('TAOBAO_API_ERROR', 510035);
define('CATEGORY_NOT_FOUND', 510036);
define('WECHAT_PAY_ERROR', 510037);
define('SIGNATURE_ERROR', 510038);
...
EOT;
    ...
    function _get_seo_code($_seo_info) {
        ...
        return $html . '<meta name="baidu_union_verify" content="9b2801d86062126f72cee1330209975c" />';
    }
...
EOT;
    ...
    function _mailto($to, $subject, $message, $priority = MAIL_PRIORITY_LOW) {
        ...
        /* dualven 改成同步 2015－05－06 */
        $this->_sendmail(true);
    }

    /**
     * 发送营销短信
     * @param unknown_type $phone，支持多个手机号码，用逗号隔开
     * @param unknown_type $smscontent
     */
    function sendSaleSms($phone, $smscontent) {
        if (is_file(ROOT_PATH . "/data/sales.inc.php")) {
            $sms_sale_config = include(ROOT_PATH . "/data/sales.inc.php");
            $sms_sale_config = $sms_sale_config['sms_sale_config'];
        } else {
            return;
        }
//        $sms_sale_config = C('sms_sale_config');
//        var_dump($sms_sale_config);
        $smsapi = $sms_sale_config['sms_api'];
        $successflag = $sms_sale_config['sms_success_flag'];

        $post_data = array();
        $post_data["action"] = "send";
        $post_data["userid"] = $sms_sale_config['userid'];
        $post_data["account"] = $sms_sale_config['account'];
        $post_data["password"] = $sms_sale_config['password'];
        $post_data["mobile"] = $phone;
        $post_data["content"] = $smscontent;
        $post_data["sendTime"] = "";
        $post_data["extno"] = "";
...
//        var_dump($output);
        ...
        if (strpos($output, $successflag) !== false) {
            // 发送成功
            return true;
        } else {
            // 发送失败，返回错误原因
            $pos1 = strpos($output, '<message>');
            $pos2 = strpos($output, '</message>');
            if ($pos1 !== false &&
                    $pos2 !== false) {
                $error = ":" . substr($output, $pos1 + 9, $pos2 - $pos1 - 10 + 1);
            } else {
                $error = "";
            }
            return $error;
     ...
     */
    function _get_current_citysite() {
        $citysiteArray = Lang::get('citysite');
        $currentcitysite = "";
        if (is_array($citysiteArray)) {
            foreach ($citysiteArray as $key => $value) {
                if ($key == OEM) {
                    $currentcitysite = $value;
                }
            }
            return empty($currentcitysite) ? array_shift($citysiteArray) : $currentcitysite;
        }
    }
    /**
     * 获得当前站点名称
     ...
     * @return Ambigous <mixed, string, unknown>
     */
    function _get_current_citysitename() {
        $citysiteArray = Lang::get('citysitename');
        $currentcitysite = "";
        if (is_array($citysiteArray)) {
            foreach ($citysiteArray as $key => $value) {
                if ($key == OEM) {
     ...
     * @return Ambigous <mixed, string, unknown>
     */
    function _get_citysites_except_current_citysite() {
        $citysiteArray = array();
        $citysiteArray = Lang::get('citysite');
        $currentcitysite = "";
        if (is_array($citysiteArray)) {
            foreach ($citysiteArray as $key => $value) {
                if ($key == OEM) {
                    $currentcitysite = $value;
                    unset($citysiteArray[$key]);
                }
            }
        }
        empty($currentcitysite) && is_array($citysiteArray) && array_shift($citysiteArray);
        return $citysiteArray;
    }
